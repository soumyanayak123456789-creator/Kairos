...
SYSTEM_PROMPT = (
    ...
    "in one step.\n"
    "PACE LIKE A HUMAN — distribute by deadline runway: when the deadline is "
    "comfortably far and there are plenty of free days before it, SPREAD the blocks "
    "across those days and AVOID cramming many hours back-to-back in a single day — "
    "prefer a reasonable daily load with gaps/breaks between long blocks, the way a "
    "person would pace themselves. When the deadline is TIGHT and free time is "
    "scarce, it is correct and EXPECTED to pack blocks densely (even back-to-back) "
    "to fit the work — do NOT refuse or under-book just to keep the day relaxed. "
    "Judge this from the available free time vs. the total effort vs. the deadline: "
    "spread-and-breathe when there is room, pack when there isn't. This affects only "
    "HOW the blocks are distributed across days — NOT how much total work you "
    "schedule (still cover the whole estimated effort), NOT the number of subtasks, "
    "and NEVER the working hours or per-run caps.\n"
...
def run_agent(sub: str, data: dict, goal: str | None, deadline: str | None, hours: int) -> dict:
    ...
    policy = (
        ...
        f"Pace by deadline runway: when the deadline is comfortably far with plenty of "
        f"free days, SPREAD blocks across those days with gaps and avoid cramming many "
        f"hours back-to-back in one day; when free time is tight before the deadline, "
        f"packing densely is correct and expected — never under-book just to keep the "
        f"day relaxed. This changes only how blocks spread across days, not the total "
        f"work, the subtasks, the working hours, or the caps. "
